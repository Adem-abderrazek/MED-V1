...
export default function PatientProfileSettingsScreen() {
  ...
  // Get theme colors based on actual user type from profile
  // Use useMemo to recalculate when profile changes
  const { colors, gradientColors, primaryGradient } = useMemo(() => {
    const userType = profile?.userType as 'medecin' | 'tuteur' | 'patient' | null;
