...
import { formatTime, formatDate, formatDateTime } from '../../utils/formatting/timeFormatting';
  ...
  const loadPatientData = useCallback(async () => {
    ...
    try {
      ...
      if (voiceResult.success) {
        ...
        console.log('🔍 First voice message details:', voiceData?.[0]);
        console.log('🔍 Title check:', voiceData?.[0]?.title);
        console.log('🔍 FileName check:', voiceData?.[0]?.fileName);
...
const styles = StyleSheet.create({
  ...
  // Loading Container (duplicate removed - using the one above)
  // loadingText (duplicate removed - using the one above)
});
