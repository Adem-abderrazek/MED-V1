...
export default function DoctorDashboardScreen() {
  ...
  const loadPatients = async () => {
    ...
    try {
      ...
      if (!token) {
        showCustomModal(
          t('common.error'),
          t('dashboard.doctor.sessionExpired'),
          'error',
          {
            text: t('auth.signIn'),
      ...
      if (result.success && result.data && Array.isArray(result.data)) {
        ...
        showCustomModal(
          t('common.error'),
          result.message || t('dashboard.doctor.loadPatientsError'),
          'error'
        );
        setPatients([]);
      }
      ...
      showCustomModal(
        t('common.error'),
        t('dashboard.doctor.errorLoadingPatients'),
        'error'
      );
    } finally {
      setIsLoading(false);
  ...
  const handleAddPatient = useCallback(() => {
    router.push('/shared/add-patient' as any);
  }, [router]);

  const handleViewProfile = useCallback((patient: Patient) => {
    router.push({
      pathname: '/patient/patient-profile' as any,
  ...
  const confirmDeletePatient = useCallback(async (patient: Patient) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showCustomModal(
          t('common.success'),
          `${patient.firstName} ${patient.lastName} ${t('dashboard.doctor.patientDeleted') || 'has been deleted from your patient list.'}`,
          'success'
        );
      } else {
      ...
      showCustomModal(
        t('common.error'),
        error.message || t('dashboard.doctor.deletePatientError'),
        'error'
      );
    } finally {
      setIsDeleting(false);
  ...
  }, [token, router, showCustomModal, t, loadPatients, loadDashboardData]);
  ...
  useFocusEffect(
    useCallback(() => {
      // Only refresh if token is ready
      if (!isTokenLoading && token) {
  ...
  const clearSearch = useCallback(() => {
    setSearchQuery('');
  }, []); // No dependencies needed - simple setter
        ...
        <View style={styles.headerContent}>
          <View style={styles.headerTop}>
            ...
            <View style={styles.headerButtons}>
              <TouchableOpacity style={styles.profileButton} onPress={() => router.push('/doctor/doctor-profile' as any)}>
  ...
  const renderEmptyState = useCallback(() => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptySubtitle}>
        {searchQuery ? t('dashboard.doctor.noPatients') : t('dashboard.doctor.startByAdding')}
      </Text>
    </View>
  ), [searchQuery]);
